...
import com.mike.aframe.database.utils.OneToMany;
...
public class KJDB {
    ...
    private KJDB(DaoConfig config) {
        if (config == null) {
            throw new MKException("daoConfig is null");
        }
        if (config.getContext() == null) {
            throw new MKException("android context is null");
        }
    ...
    private SQLiteDatabase createDbFileOnSDCard(String sdcardPath,
        ...
        if (!dbf.exists()) {
            try {
                ...
                throw new MKException("数据库文件创建失败", ioex);
            }
        } else {
