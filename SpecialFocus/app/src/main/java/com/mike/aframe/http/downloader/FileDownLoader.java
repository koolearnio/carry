 ...
 */
package com.mike.aframe.http.downloader;
...
import com.mike.aframe.core.SparseIntArray;
import com.mike.aframe.http.I_HttpRespond;
import com.mike.aframe.ui.KJActivityManager;
...
public class FileDownLoader implements I_MulThreadLoader {
    ...
    public FileDownLoader(String _url, File saveFile, int threadNum) {
        ...
        if (saveFile.isDirectory()) {
            throw new MKException("absFilePath is directory");
        }
        if (!saveFile.exists()) {
            try {
                saveFile.createNewFile();
            } catch (IOException e) {
                throw new MKException("absFilePath can not create");
            }
        }
    ...
    private void initialize(String _url) {
        try {
            ...
            if (conn.getResponseCode() == 200) {
                ...
                if (this.fileSize <= 0) {
                    throw new MKException("Unkown file size ");
                }
                ...
                throw new MKException("server response code is: "
                        + conn.getResponseCode());
            }
        } catch (MalformedURLException e) {
            throw new MKException("do not connection this url", e);
        } catch (IOException e) {
            throw new MKException("connection error", e);
        }
    }
    ...
    private URL initFile() {
        ...
        try {
            ...
            throw new MKException("fail :saveFile not found", e);
        } catch (IOException e) {
        ...
        try {
            ...
            throw new MKException("fail :url error", e);
        }
        return url;
    }
    ...
    private void respondFailIfError(DownloadThread thread) {
        if (thread != null && thread.isError()) {
            throw new MKException("download error: IOException");
        }
    }
