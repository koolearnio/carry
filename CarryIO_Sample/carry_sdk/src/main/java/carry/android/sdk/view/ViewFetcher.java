...
public class ViewFetcher {
     ...
     * @return
     */
    public ArrayList<CView> getAllViewsWithId(boolean onlySufficientlyVisible){
        final ArrayList<CView> allKDViews = new ArrayList<>();
        final View[] views = getWindowDecorViews();
        final View[] nonDecorViews = getNonDecorViews(views);
        View view = null;
        ...
        if(nonDecorViews != null){
            for(int i = 0; i < nonDecorViews.length; i++){
                view = nonDecorViews[i];
                try {
                    addCViewChildren(allKDViews, new CViewGroup((ViewGroup)view,view.getClass().getSimpleName()), onlySufficientlyVisible);
                } catch (Exception ignored) {

                }
                if(view != null)
                    allKDViews.add(
                            new CView(view.getClass().getSimpleName(),
                                    view.getId(),view));
            }
        }
    ...
    private void addCViewChildren(ArrayList<CView> views, CViewGroup viewGroup, boolean onlySufficientlyVisible) {
        if(viewGroup != null){
            for (int i = 0; i < viewGroup.getmViewGroup().getChildCount(); i++) {
                ...
                if(onlySufficientlyVisible && isViewSufficientlyShown(child)) {
                    views.add(new CView(childViewPath,child.getId(),child));
                }
                ...
                if (child instanceof ViewGroup) {
                    addCViewChildren(views, new CViewGroup((ViewGroup)child,childViewPath), onlySufficientlyVisible);
                }
            }
        }
    }
    ...
    public View getRecyclerView(boolean shouldSleep, int recyclerViewIndex){
        ...
        ArrayList<View> views = CarryUtils.filterViewsToSet(new Class[] {ViewGroup.class}, getAllViews(false));
        views = CarryUtils.removeInvisibleViews(views);
...
//        }
        ...
        @SuppressWarnings("unchecked")
        ArrayList<View> views = CarryUtils.filterViewsToSet(new Class[] {ViewGroup.class}, getAllViews(true));
        views = CarryUtils.removeInvisibleViews(views);
    ...
    public View getIdenticalView(View view) {
        ...
        List<? extends View> visibleViews = CarryUtils.removeInvisibleViews(getCurrentViews(view.getClass(), true));
